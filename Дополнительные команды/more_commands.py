...
def execute_chown(vfs, tokens):
    #изменяет владельца файла директории
    if len(tokens)!=3:
        print("Использование: chown <владелец> <файл>")
        return False
    owner = tokens[1] #получаем имя нового владельца
    path = tokens[2] #путь к фалу, директории
    path_parts = vfs._split_path(path)
    target = vfs._resolve_path(path_parts)
    if not target:
...
def execute_cp(vfs, tokens):
    #копирует файл или директорию
    if len(tokens)!=3:
        print(f"Использование: cp <источник> <назначение>")
        return False
    source_path = tokens[1] #путь к исходному файлу
    dest_path = tokens[2] #путь к назначению
    source_parts = vfs._split_path(source_path)
    source_node = vfs._resolve_path(source_parts)
    if not source_node:
        print(f"Ошибка: источник {source_path} не найден")
        return False
    dest_parts = vfs._split_path(dest_path)
    dest_name = dest_parts[-1] if dest_parts else ""
    parent_dest_parts = dest_parts[:-1]
    parent_dest = vfs._resolve_path(parent_dest_parts, create_misiing = False)
    if not parent_dest:
        print(f"Ошибка: директория назначения не найдена")
        return False
    if not parent_dest.is_directory:
        print(f"Ошибка: {dest_path} не является директорией")
        return False
    if dest_name in parent_dest.content:
    ...
    try:
        if not source_node.is_directory:
            new_node = VFSNode(
                name=dest_name,
                is_directory=False,
                content = source_node.content, #копируем содержимое
                parent = parent_dest
            )
            parent_dest.content[dest_name] = new_node
            vfs.total_files += 1
        else:
            new_node = VFSNode(
                name = dest_name,
            ...
            parent_dest.content[dest_name] = new_node
            vfs.total_dirs += 1
            _copy_directory_contents(vfs, source_node, new_node)
        print(f"{source_path} скопирован в {dest_path}")
        return True
    except Exception as e:
        print(f"Ошибка копирования: {e}")
        return False
def _copy_directory_contents(vfs, source_dir, dest_dir):
    for name, node in source_dir.content.items():
        if node.is_directory:
            ...
            dest_dir.contents[name] = new_subdir
            vfs.total_dirs += 1
            _copy_directory_contents(vfs, node, new_subdir)
        else:
            new_file = VFSNode(
                name = name,
                is_directory = False,
            ...
            dest_dir.content[name] = new_file
            vfs.total_files += 1
...
def run(vfs, script_mode = False, script_lines = None):
    ...
    while True:
        ...
        if command == "exit":
            ...
            print("  chown <owner> <file> - изменить владельца файла")
            print("  cp <src> <dest>      - копировать файл или директорию")
            ...
            execute_chown(vfs, parsed_tokens)
        elif command == "cp":
            execute_cp(vfs, parsed_tokens)
        else:
